...
public class ADHelper
{
    ...
    private string ADPath = "LDAP://DCIM1.dci.daikin.co.jp";
    private string ADServer = "DCIM1.dci.daikin.co.jp";
    private string ADUser = "administrator";
    private string ADPassword = " dceiaoahpea";
    #endregion
    ...
    public ADHelper()
    {
        ...
        //try
        //{
        //    ADPath = WebConfigurationManager.AppSettings["ADPath"].ToString();
        //}
        //catch
        //{
        //}

        try { ADPath = ConfigurationManager.AppSettings["ADPath"].ToString(); } catch { }
        ADServer = ADPath.Replace("LDAP://", "");
    }
    ...
    public DataSet GetUserDataSetByEmpCode(string empCode)
    {
        DirectoryEntry de = GetDirectoryObject();
        //create instance fo the direcory searcher
        DirectorySearcher deSearch = new DirectorySearcher();
        ...
        deSearch.SearchRoot = de;
        //set the search filter
        deSearch.Filter = "(&(objectClass=user)(postalCode=" + empCode + "))";
        deSearch.SearchScope = SearchScope.Subtree;
    ...
    /// <param name="UserName"></param>
    /// <returns></returns>
    public DataSet GetUserDataSetbyEmail(string Email)
    {
        DirectoryEntry de = GetDirectoryObject();
        //create instance fo the direcory searcher
        DirectorySearcher deSearch = new DirectorySearcher();
        ...
        {
            deSearch.Filter = "(&(objectClass=contact)(mail=" + Email + "))";
            deSearch.SearchScope = SearchScope.Subtree;
    ...
    public DataSet GetGroups()
    {
        ...
        tbGroup.Columns.Add("GroupEmail");
        tbGroup.TableName = "TbGroup";
        ...
        if (results.Count > 0)
        {
            ...
            foreach (SearchResult Result in results)
            {
                ...
                try
                {
                    rwGroup["GroupName"] = GetProperty(Result, "cn");
                }
                catch
                {
                ...
                try
                {
                    rwGroup["GroupEmail"] = GetProperty(Result, "MAIL"); // Result.Properties["Mail"][0].ToString();
                }
                catch
                {
    ...
    public DataSet GetUsersForGroup(string GroupName)
    {
        ...
        if (results != null)
        {
            ...
            for (int l = 0; l < n; l++)
            {
                string temp = pcoll["member"][l].ToString();
    ...
    private DataRow PopulateUserDataSet(SearchResult userSearchResult, DataTable userTable)
    {
        ...
        rwUser["MiddleInitial"] = GetProperty(userSearchResult, "displayname");
        ...
        rwUser["Title"] = GetProperty(userSearchResult, "homephone");
