...
public class CGeneralDDL
{
    ...
    public void GetDDLYear (DropDownList ddlYear)
    {
     ...
     public void GetDDLCostCenter(DropDownList ddlCostCenter)
    {
        SqlCommand sql = new SqlCommand();
        sql.CommandText = "SELECT  [Cost_Center],[CC_Name],[Section],[Department] " +
                        "FROM [dbBCS].[dbo].[CC_Mstr] where cost_center < '9000'";
        //DataTable dtCostCenter = oConn.SqlGet(sql, "DBBCS");
        //ddlCostCenter.DataSource = dtCostCenter;
        //ddlCostCenter.DataValueField = "Cost_Center";
        //ddlCostCenter.DataTextField = "CC_Name";
        ...
        foreach (DataRow item in dtCostCenter.Rows)
        {
            ddlCostCenter.Items.Add(item["Cost_Center"].ToString() + ":" + item["CC_Name"].ToString());
          
            //ddlCostCenter.CostCenter = item["Model"].ToString();
            //ddlCostCenter.CCName = item["ModelCode"].ToString();

            //ddlCostCenter.Items.Add(i.ToString());
        }
        ddlCostCenter.Items.Add("ALL");
        //return ddlCostCenter;
