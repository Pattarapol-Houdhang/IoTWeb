...
public class ConnectDBETD
{
    private bool useDB = true;
    //Private serverName As String = "(Local)\sqlexpress"
    //Private databaseName As String = "SYSQP"
    //private string connStr = System.Configuration.ConfigurationManager.ConnectionStrings["dbIoT"].ConnectionString;
    //=================================================================================
    //Constructor
    //=================================================================================
    ...
    /// <remarks></remarks>
    public DataTable Query(string queryStr, string connStr)
    {
        if (useDB)
        {
    ...
    /// <returns>DataTable</returns>
    /// <remarks></remarks>
    public DataTable Query(SqlCommand commandDb, string connStr)
    {
    ...
    public void ExecuteCommand(string exeStr, string connStr)
    {
        if (useDB)
        {
            SqlConnection conn = new SqlConnection(connStr);
            try
            {
                ...
                ExecuteCommand(commandDb, connStr);
            }
            catch (Exception ex)
            {
                HttpContext.Current.Session["QueryError"] = ex.ToString();
    ...
    /// <remarks></remarks>
    public void ExecuteCommand(SqlCommand commandDb, string connStr)
    {
        if (useDB)
        {
            SqlConnection conn = new SqlConnection(connStr);
    ...
    /// <remarks></remarks>
    public void ExecuteCommand(List<string> exeStr, string connStr)
    {
        if (useDB)
        {
            SqlConnection conn = new SqlConnection(connStr);
    ...
    /// <remarks></remarks>
    public void ExecuteCommand(List<SqlCommand> commandDb, string connStr)
    {
        if (useDB)
        {
            SqlConnection conn = new SqlConnection(connStr);
