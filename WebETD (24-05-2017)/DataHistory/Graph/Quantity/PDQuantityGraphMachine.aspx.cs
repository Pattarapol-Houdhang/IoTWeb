...
public partial class Production_Graph_Quantity_PDQuantityGraphMachine : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        {
            Response.Redirect(ResolveClientUrl("~/DataHistory/Graph/Quantity/PDQuantityGraphLine.aspx?FactoryID" + ViewState["FactoryID"].ToString()));
        }
    ...
    public string GenBlock()
    {
        ...
        if (oMDMachine.ListOfmachine.Count > 0)
        {
            foreach (MDMachine.CMDMachine oMD in oMDMachine.ListOfmachine)
            {
                ...
                result += "            <a href='" + ResolveClientUrl("~/DataHistory/Graph/Quantity/PDQuantityGraph.aspx?ld_id=" + ViewState["ld_id"].ToString()) + "&FactoryID=" + ViewState["FactoryID"].ToString() + "&mc_code=" + oMD.mc_code + "'><i class='icon-tree'></i>" + Environment.NewLine;
