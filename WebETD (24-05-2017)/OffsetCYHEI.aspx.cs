...
public partial class OffsetCYHEI : System.Web.UI.Page
{
    private string bufId;
    OffsetMasterService offSer;
    private string _partID = "3";
    private string _partTypeID = "10";
        ...
        if (!IsPostBack)
        {
            if ((bufId = Request.QueryString["id"]) != null)
            {
                ...
                foreach (OffsetMasterInfo offsetInfo in _OffsetDataList)
                {
                    if (counter == 0)
                    {
                        txtH1.Text = offsetInfo.Offset_value.ToString();

                    }
                    else if (counter == 1)
                    {

                        txtH2.Text = offsetInfo.Offset_value.ToString();

                    }
                    else if (counter == 2)
                    {

                        txtH3.Text = offsetInfo.Offset_value.ToString();

                    }
                    else if (counter == 3)
                    {

                        txtH4.Text = offsetInfo.Offset_value.ToString();
    ...
    {
        if (txtMasterNo.Text != "" && txtH1.Text != "" && txtH2.Text != "" && txtH3.Text != "" && txtH4.Text != "")
        {
            ...
            if (Convert.ToBoolean(Session["editState"]) == false)
            {
                for (int i = 0; i < 4; i++)
                {
                    ...
                    if (i == 0)
                    {
                        offInfo.Offset_value = Convert.ToDouble(txtH1.Text);
                    ...
                    {
                        offInfo.Offset_value = Convert.ToDouble(txtH2.Text);
                        if (offSer.InsertOffsetData(offInfo))
                        {
                            offSer.InsertOffsetDataLow(offInfo);
                            txtH2.Text = "";
                    ...
                    {
                        offInfo.Offset_value = Convert.ToDouble(txtH3.Text);
                        if (offSer.InsertOffsetData(offInfo))
                        {
                            offSer.InsertOffsetDataLow(offInfo);
                            txtH3.Text = "";
                    ...
                    {
                        offInfo.Offset_value = Convert.ToDouble(txtH4.Text);
                        if (offSer.InsertOffsetData(offInfo))
                        {
                            offSer.InsertOffsetDataLow(offInfo);
                            txtH4.Text = "";
