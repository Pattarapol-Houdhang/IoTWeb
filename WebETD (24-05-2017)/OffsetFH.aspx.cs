...
public partial class OffsetFH : System.Web.UI.Page
{
    private string bufId;
    OffsetMasterService offSer;
    private string _partID = "4";
            ...
            if ((bufId = Request.QueryString["id"]) != null)
            {
                ...
                foreach (OffsetMasterInfo offsetInfo in _OffsetDataList)
                {
                    ...
                    else if (counter == 2)
                    {
                        ...
                        txtC1.Text = offsetInfo.Offset_value.ToString();
    ...
    {
        if (txtMasterNo.Text != "" && txtID1.Text != "" && txtID2.Text != "" && txtC1.Text != "" && txtP1.Text != "")
        {
            ...
            if (Convert.ToBoolean(Session["editState"]) == false)
            {
                for (int i = 0; i < 4; i++)
                {
                    ...
                    {
                        offInfo.Offset_value = Convert.ToDouble(txtC1.Text);
                        if (offSer.InsertOffsetData(offInfo))
                        {
                            txtC1.Text = "";
                        }
