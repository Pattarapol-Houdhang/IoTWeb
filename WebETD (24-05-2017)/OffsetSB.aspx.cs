...
public partial class OffsetSW : System.Web.UI.Page
{
    private string bufId;
    OffsetMasterService offSer;
    private string _partID = "6";
    private string _partTypeID = "13";
        ...
        if (!IsPostBack)
        {
            if ((bufId = Request.QueryString["id"]) != null)
            {
                ...
                foreach (OffsetMasterInfo offsetInfo in _OffsetDataList)
                {
                    if (counter == 0)
                    {
                        txtT1.Text = offsetInfo.Offset_value.ToString();

                    }
                    else if (counter == 1)
                    {

                        txtT2.Text = offsetInfo.Offset_value.ToString();
    ...
    {
        if (txtMasterNo.Text != "" && txtT1.Text != "" && txtT2.Text != "")
        {
            ...
            if (Convert.ToBoolean(Session["editState"]) == false)
            {
                for (int i = 0; i < 2; i++)
                {
                    ...
                    if (i == 0)
                    {
                        offInfo.Offset_value = Convert.ToDouble(txtT1.Text);
                    ...
                    {
                        offInfo.Offset_value = Convert.ToDouble(txtT2.Text);
                        if (offSer.InsertOffsetData(offInfo))
                        {
                            txtT2.Text = "";
                        }
