...
public partial class Production_FinalLine_ReportReturnDataByLabelLine1 : System.Web.UI.Page
{
    ...
    protected void txtSerialNo_TextChanged(object sender, EventArgs e)
    {
        ...
        {
            DataTable dt = GetDataHistory(txtSerialNo.Text.Trim().Substring(0, 15).ToUpper());
    ...
    protected void btnTransfer_Click(object sender, EventArgs e)
    {
        ...
        if (txtSerialNo.Text.Trim().Length >= 15) {
            string _SerialNo = "";
            _SerialNo = txtSerialNo.Text.Substring(0,15).Trim();
            //string _YearNew = _SerialNew.Substring(1, 1);
            //string _MonthNew = _SerialNew.Substring(2, 1);
            string _DataModelNo = txtSerialNo.Text.Substring(4, 3);
            ...
            {
                DataTable dtLabel2 = new DataTable();
                ...
                cmdLabel2.Parameters.Add(new SqlParameter("SerialNumber", _SerialNo));
                dtLabel2 = oConDCI.Query(cmdLabel2);
                if (dtLabel2.Rows.Count > 0)
                {
                    _Line = "2";
                }
            }
            ...
            if (_Line != "0")
            {
                ...
                string strModel = @"SELECT TOP 1 Model, rmk1 
                                FROM PN_Compressor 
                                WHERE [Status]='ACTIVE' AND Line IN ('1','2') AND ModelCode=@ModelCode ";
                ...
                //----------- DELETE FROM REWORK ---------
                string SQLDelete = @"DELETE FROM FG_ReportDefect WHERE SerialNo=@SerialNo AND dataLine=@dataLine ";
                SqlCommand cmdDelete = new SqlCommand();
                cmdDelete.CommandText = SQLDelete;
                cmdDelete.CommandTimeout = 180;
                cmdDelete.Parameters.Add(new SqlParameter("@dataLine", _Line));
                cmdDelete.Parameters.Add(new SqlParameter("@SerialNo", _SerialNo));
                oConIoT.ExecuteCommand(cmdDelete);



                //----------- INSERT TO RETURN ---------
                string SQLInsert = @"INSERT INTO FG_ReportReturn (dataDate,dataShift,dataLine,
                                            dataModelCode,dataModel,SerialNo,ReportBy,ReportDate) VALUES 
                                            (@dataDate,@dataShift,@dataLine,@dataModelCode,@dataModel,
                                             @SerialNo,@ReportBy,@ReportDate) ";
    ...
    private DataTable GetDataHistory(string PrdSerial)
    {
        ...
        if (PrdSerial.Trim().Length == 15)
        {
            ...
            if (_Line != "0")
            {
                ...
                string strQuery = @"SELECT Nbr,dataDate,dataShift,dataLine,dataModelCode,dataModel,
                                    SerialNo,ReportBy,ReportDate, 
                                    CONCAT([dataModelCode],' : ',[dataModel]) AS Model
                                FROM FG_ReportReturn 
