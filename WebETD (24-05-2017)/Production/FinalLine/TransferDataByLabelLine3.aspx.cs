...
public partial class Production_FinalLine_TransferDataByLabelLine3 : System.Web.UI.Page
{
            ...
            if (rptSerialOld.Items.Count > 0)
            {
                if (isCanTransferLabel(txtSerialOld.Text.Trim().Substring(0, 15)))
                {
                    ...
                    if (_DataModelNew == _DataModelOld)
                    {
                        ...
                        string SQLOil = @"SELECT L3O_ID, SerialNo, ModelNo, ModelName, BeforeWeight, 
                            AfterWeight, OilChargeValue, Judgement, InsertDate, MFGDate
                        FROM Line3_OilCharge 
                        ...
                        if (dtOil.Rows.Count > 0)
                        {
                            foreach (DataRow drOil in dtOil.Rows)
                            {
                                try
                                {
                                    string SQLInsert = @"INSERT INTO Line3_OilCharge (SerialNo, ModelNo, 
                                ...
                                try
                                {
                                    ...
                                    cmdLog.Parameters.Add(new SqlParameter("@DataValue14", "LINE 3"));
                            ...
                            string StrRunTest = @"SELECT L32YC_ID, SerialNo, MachineNo, ModelNo, ModelName, 
                                ...
                                RunningJudgement, VacuumCapJudgement, VacuumLeakJudgement, DischargeJudgement, 
                                TotalJudgement, InsertDate, MFGDate
                            FROM Line3_RunningTest 
                            WHERE (SerialNo = @SerialNo)  
                            ORDER BY MFGDate DESC ";
                            ...
                            if (dtRunTest.Rows.Count > 0)
                            {
                                foreach (DataRow drRunTest in dtRunTest.Rows)
                                {
                                    try
                                    {
                                        string SQLInsert = @"INSERT INTO Line3_RunningTest 
                                        (SerialNo, MachineNo, ModelNo, ModelName, CurrentLower, CurrentValue, 
                                        CurrentUpper, VacuumLower, VacuumValue, VacuumUpper, LeakValue, LeakUpper, 
                                        BarCodeJudgement, PressureJudgement, FrequencyJudgement, RunningJudgement, 
                                        VacuumCapJudgement, VacuumLeakJudgement, DischargeJudgement, TotalJudgement, 
                                        InsertDate, MFGDate) 
                                        VALUES (@SerialNo, @MachineNo, @ModelNo, @ModelName, @CurrentLower, 
                                        @CurrentValue, @CurrentUpper, @VacuumLower, @VacuumValue, @VacuumUpper, 
                                        @LeakValue, @LeakUpper, @BarCodeJudgement, @PressureJudgement, 
                                        @FrequencyJudgement, @RunningJudgement, @VacuumCapJudgement, 
                                        @VacuumLeakJudgement, @DischargeJudgement, @TotalJudgement, 
                                        @InsertDate, @MFGDate) ";
                                        SqlCommand cmdInsert = new SqlCommand();
                                        cmdInsert.CommandText = SQLInsert;
                                        cmdInsert.CommandTimeout = 180;
                                        ...
                                        cmdInsert.Parameters.Add(new SqlParameter("@DischargeJudgement", drRunTest["DischargeJudgement"].ToString()));
                            ...
                            string StrLowVoltage = @"SELECT L3HV_ID, SerialNo, ModelNo, ModelName, 
                            ...
                            if (dtLowVoltage.Rows.Count > 0)
                            {
                                foreach (DataRow drLowVoltage in dtLowVoltage.Rows)
                                {
                                    try
                                    {
                                        string SQLInsert = @"INSERT INTO Line3_HVTest (SerialNo, 
                            ...
                            try
                            {
                                ...
                                sqlApp.CommandText = strSQLApp;
                                sqlApp.Parameters.Add(new SqlParameter("@Serial_No", txtSerialOld.Text.Trim()));
                                dTable = oConIoT.Query(sqlApp);
                            ...
                            string StrWeight = @"SELECT L3W_ID, SerialNo, ModelNo, ModelName, 
                                WeightCheckJudgement, WeightMeasured, TerminalCover, InsertDate, MFGDate 
                            FROM Line3_WeightCheck 
                            WHERE SerialNo=@SerialNo 
                            ORDER BY MFGDate DESC ";
                            ...
                            if (dtWeight.Rows.Count > 0)
                            {
                                foreach (DataRow drWeight in dtWeight.Rows)
                                {
                                    try
                                    {
                                        string SQLInsert = @"INSERT INTO Line3_WeightCheck (SerialNo, ModelNo, 
