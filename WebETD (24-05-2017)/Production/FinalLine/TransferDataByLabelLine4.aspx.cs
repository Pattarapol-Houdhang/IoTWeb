...
public partial class Production_FinalLine_TransferDataByLabelLine4 : System.Web.UI.Page
{
    ...
    protected void rptSerialNew_ItemDataBound(object sender, RepeaterItemEventArgs e)
    {
        if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
        {
            ...
            Literal ltrApp = (Literal)e.Item.FindControl("ltrApp") as Literal; //Edit by Earth 2020-04-13 11:54
    ...
    protected void btnTransfer_Click(object sender, EventArgs e)
    {
        ...
        if (txtSerialNew.Text.Trim().Length >= 11 && txtSerialOld.Text.Trim().Length >= 11)
        {
            ...
            if (rptSerialOld.Items.Count > 0)
            {
                ...
                if (txtSerialNew.Text.Trim().Length == 11)
                {
                    _DataModelNew = _SerialNew.Substring(0, 4);
                    _DataModelOld = _SerialOld.Substring(0, 4);
                }
                else
                {
                    _DataModelNew = _SerialNew.Substring(3, 4);
                    _DataModelOld = _SerialOld.Substring(3, 4);
                }
                ...
                if (_DataModelNew == _DataModelOld)
                {
                    ...
                    if (PipeNew == PipeOld)
                    {
                        ...
                        string StrRunTest = @"select [SCRID],[SerialNo],[ModelNo],[ModelNoJudgement],[ModelName],[SoundCheck],[SoundCheckJudement],[Power],[PowerJudgement],[CompTime]
                                              ,[CompTimeJudgement],[N2Suction],[N2SuctionJudgement],[InverseFlow],[InverseFlowJudgement],[PrimaryTankPS],[OutletPS],[InletPS]
                                              ,[Voltage],[Ampere],[PowerCheck],[Frequency] ,[StepNo],[MotorType],[Phase],[Voltage2],[Motor],[Freq],[Unload],[YC_CurrentLower]
                                              ,[YC_CurrentMeasure],[YC_CurrentUpper],[YC_CurrentJudgement],[YC_VacuumLower],[YC_VacuumMeasure],[YC_VacuumUpper],[YC_VacuumJudgement]
                                              ,[YC_LeakMeasure],[YC_LeakUpper],[YC_LeakJudgement],[YC_PressureMeasure],[YC_PressureUpper],[YC_PressureJudgement],[YC_SoundJudge]
                                              ,[YC_PressurePurge],[YC_TotalJudge],[InsertDate],[MachineNo],[MFGDate]
...
//                                    cmdInsert.Parameters.Add(new SqlParameter("@MachineNo", drRunTest["MachineNo"].ToString()));
                                    ...
                                    #region New code Earth
                                    string SQLInsert = @"INSERT INTO SCR_runningtest 
                                                (SerialNo,[ModelNo],[ModelNoJudgement],[ModelName],[SoundCheck],[SoundCheckJudement],[Power],[PowerJudgement],[CompTime]
                                              ,[CompTimeJudgement],[N2Suction],[N2SuctionJudgement],[InverseFlow],[InverseFlowJudgement],[PrimaryTankPS],[OutletPS],[InletPS]
                                              ,[Voltage],[Ampere],[PowerCheck],[Frequency] ,[StepNo],[MotorType],[Phase],[Voltage2],[Motor],[Freq],[Unload],[YC_CurrentLower]
                                              ,[YC_CurrentMeasure],[YC_CurrentUpper],[YC_CurrentJudgement],[YC_VacuumLower],[YC_VacuumMeasure],[YC_VacuumUpper],[YC_VacuumJudgement]
                                              ,[YC_LeakMeasure],[YC_LeakUpper],[YC_LeakJudgement],[YC_PressureMeasure],[YC_PressureUpper],[YC_PressureJudgement],[YC_SoundJudge]
                                              ,[YC_PressurePurge],[YC_TotalJudge],[InsertDate],[MachineNo],[MFGDate])
                                              (SELECT @New_Serial,[ModelNo],[ModelNoJudgement],[ModelName],[SoundCheck],[SoundCheckJudement],[Power],[PowerJudgement],[CompTime]
                                              ,[CompTimeJudgement],[N2Suction],[N2SuctionJudgement],[InverseFlow],[InverseFlowJudgement],[PrimaryTankPS],[OutletPS],[InletPS]
                                              ,[Voltage],[Ampere],[PowerCheck],[Frequency] ,[StepNo],[MotorType],[Phase],[Voltage2],[Motor],[Freq],[Unload],[YC_CurrentLower]
                                              ,[YC_CurrentMeasure],[YC_CurrentUpper],[YC_CurrentJudgement],[YC_VacuumLower],[YC_VacuumMeasure],[YC_VacuumUpper],[YC_VacuumJudgement]
                                              ,[YC_LeakMeasure],[YC_LeakUpper],[YC_LeakJudgement],[YC_PressureMeasure],[YC_PressureUpper],[YC_PressureJudgement],[YC_SoundJudge]
                                              ,[YC_PressurePurge],[YC_TotalJudge],[InsertDate],[MachineNo],[MFGDate]
                                            FROM SCR_runningtest 
                                            WHERE (SerialNo = @Old_Serial)) ";
                                    SqlCommand cmdInsert = new SqlCommand();
                                    cmdInsert.CommandText = SQLInsert;
                                    cmdInsert.CommandTimeout = 180;
                                    cmdInsert.Parameters.Add(new SqlParameter("@New_Serial", _SerialNew));
                                    cmdInsert.Parameters.Add(new SqlParameter("@Old_Serial", _SerialOld));
                                    oConIoT.ExecuteCommand(cmdInsert);
                                    #endregion
                                }
                                catch { }
                        ...
                        string SQLCenter = @"SELECT Serial, Line, LB_InsertDate, LB_MFGDate, 
                                        ...
                                        [AP_Judgement],[AP_InsertDate],[TerminalCover]
                                    FROM FN_DataCenter 
                                    WHERE Serial=@Serial ";
                        ...
                        if (dtCenter.Rows.Count > 0)
                        {
                            foreach (DataRow drCenter in dtCenter.Rows)
                            {
                                ...
                                if (dtCenterNew.Rows.Count == 0)
                                {
                                    ...
                                    strCenterInstr += " [ModelCode],[Line],[LB_InsertDate],[LB_MFGDate],[OC_Judgement], ";
                                    ...
                                    strCenterInstr += "  SELECT '" + _SerialNew + "', ModelCode, Line, LB_InsertDate, ";
                                ...
                                else
                                {
                                    try
                                    {
                                        ...
                                        //------ Appearance ------ Edit by Earth 2020-04-13 11:54
                                        ...
                                        ///check updateed by EARTH 2020-05-05 09:08
