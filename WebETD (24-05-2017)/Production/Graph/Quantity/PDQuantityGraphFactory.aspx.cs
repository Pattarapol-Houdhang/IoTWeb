...
public partial class Production_Graph_Quantity_PDQuantityGraphFactory : System.Web.UI.Page
{
    ...
    public string GenBlock()
    {
        ...
        if (oMDFactory.ListOfFactory.Count > 0)
        {
            foreach (MDFactory.CMDFactory oMD in oMDFactory.ListOfFactory)
            {
                ...
                result += "            <a href='" + ResolveClientUrl("~/Production/Graph/Quantity/PDQuantityGraphLine.aspx?FactoryID=" + oMD.FactoryID) + "'><i class='icon-factory'></i>" + Environment.NewLine;
