    ...
    public string GenBlock()
    {
        ...
        if (oMDLine.ListOfLine.Count > 0)
        {
            foreach (MDLineData.CMDLineData oMD in oMDLine.ListOfLine)
            {
                ...
                result += "            <a href='"+ ResolveClientUrl("~/Production/Graph/Quantity/PDQuantityGraphMachine.aspx?ld_id=" + oMD.ld_id + "&FactoryID=" + ViewState["FactoryID"].ToString()) + "'><i class='icon-tree'></i>" + Environment.NewLine;
