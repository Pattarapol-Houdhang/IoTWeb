    ...
    public string GenBlock()
    {
        ...
        if (oMDMachine.ListOfmachine.Count > 0)
        {
            foreach (MDMachine.CMDMachine oMD in oMDMachine.ListOfmachine)
            {
                ...
                result += "            <a href='"+ ResolveClientUrl("~/Production/Graph/Quantity/PDQuantityGraph.aspx?ld_id=" + ViewState["ld_id"].ToString()) + "&FactoryID="+ ViewState["FactoryID"].ToString() +"&mc_code="+ oMD.mc_code +"'><i class='icon-tree'></i>" + Environment.NewLine;
