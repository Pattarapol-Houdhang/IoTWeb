...
public partial class ReportRankFH : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            if ((receiveText = Request.QueryString["report"]) != null)
            {
                ...
                ptidSer = new RHIDService();
                viModelDetailSer = new VIModelDetailService();

                _idData = ptidSer.GetRHIDData(_modelID, startDate, endDate, ViewState["conStr"].ToString());
    ...
    public string resultRHID()
    {
        if (_idData != null)
        {
            ...
            chartScript += dc.SetLineChartHeader("RHID", partType, startDate + " to " + endDate, _maxVal, _minVal);
            chartScript += dc.SetLineChartSeriesData(partType, _idData, typeof(RHIDInfo), _maxVal, _midVal, _minVal);
    ...
    public string resultRHIDRoundness()
    {
        if (_idData != null)
        {
            string chartScript = "";
            ...
            chartScript += dc.SetLineChartHeaderMax("RHIDRound", partType, startDate + " to " + endDate, _maxVal, _minVal);
    ...
    public string resultRHIDCylindricity()
    {
        if (_idData != null)
        {
            string chartScript = "";
            ...
            chartScript += dc.SetLineChartHeaderMax("RHIDCy", partType, startDate + " to " + endDate, _maxVal, _minVal);
            chartScript += dc.SetLineChartSeriesDataMax(partType, _idData, typeof(RHIDInfo), _maxVal);
            chartScript += dc.SetFooterChart();
